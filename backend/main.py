...
async def simulate_processing(job_id: str, sample_dir: str):
    ...
    if os.path.exists(test_image):
        ...
        print(f"Copied test image to {image_path}")
    else:
        print(f"Test image not found, skipping copy")
    
    await asyncio.sleep(1)
    
    detection_result = detect_microplastics(image_path if os.path.exists(image_path) else "placeholder")
    ...
    print(f"Job {job_id} completed with {detection_result['detected_count']} detections")
